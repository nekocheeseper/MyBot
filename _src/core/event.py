...
async def on_presence_update(before, after, bot):
    if await dispatcher.on_presence_update(before, after, bot):
...
async def on_voice_state_update(member, before, after, bot):
    if await dispatcher.on_voice_state_update(member, before, after, bot):
        if before.channel is None and after.channel is not None:
            ...
            logger.info(f"{member.name} movsed from voice channel {before.channel} to {after.channel}")
    return